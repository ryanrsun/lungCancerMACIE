...
def readcommandline(argv):
  ...
  factors = ''
  try:
    opts, args = getopt.getopt(argv,"m:a:c:o:f:p:n:",["mode=","afile=","cfile=","ofolder=","factors","pfolder=","name="])
  except getopt.GetoptError:
    print 'usage: test.py -m <mode> -a <annotationfile> -c <classfile> -o <outputfolder> -f <factors> -p <parameterfolder> -n <outputname>'
    sys.exit(2)
  for opt, arg in opts:
    if opt in ("-m", "--mode"):
      ...
      outputname = arg
  return mode, annotationfile, classfile, outputfolder, factors, parameterfolder, outputname
...
'''
Compute difference in parameters between two Zeta's.
'''
...
'''
Combined annotation mixed models
'''
def camm(data, data_type, Zeta_0, n_factors, criteria=1e-4, max_iter=100):
  ...
  while (diff > 1e-4 and iter <= max_iter):
    if (iter % (max_iter/10) == 0):
      print str(iter*10/(max_iter/10)) + '% complete, diff=' + str(diff)
...
def save_posterior(posterior, annotationfile, classfile, directory, prefix='', suffix=''):
  ...
  snp_info.readline()
  file = open(directory+prefix+'posterior'+suffix+'.txt','w')
...
# python camm.py -m train -a Example\ I2/annotation.txt -c Example\ I2/class.txt -o Example\ I2/ -f 22
# python camm.py -m predict -a Example\ I2/annotation.txt -c Example\ I2/class.txt -o Example\ I2/  -f 22 -p Example\ I2/

if __name__ == "__main__":
  mode, annotationfile, classfile, outputfolder, n_factors, parameterfolder, outputname = readcommandline(sys.argv[1:])
  annotation, data_type, data = readfile(annotationfile, classfile)
  if mode == "train":
    Zeta_0 = find_Zeta_0(data, data_type, n_factors)
    Zeta_hat, posterior, diff, iter, parameter_r = camm(data, data_type, Zeta_0, n_factors, criteria=1e-4, max_iter=200)
    save_Zeta(Zeta_hat, n_factors, outputfolder, annotation, prefix='hat_')
    save_posterior(posterior, annotationfile, classfile, outputfolder, prefix='hat2_')
    ...
    Zeta_hatp1, posterior, diff, iter, parameter_r = camm(data, data_type, Zeta_hat, n_factors, criteria=1e-4, max_iter=1)
    save_posterior(posterior, annotationfile, classfile, outputfolder, prefix=outputname)
